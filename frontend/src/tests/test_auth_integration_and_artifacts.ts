...
console.log(
    "🧪 Starting Task 21 Full Auth Integration & Artifact Verification Suite...\n",
);
...
async function runAuthServiceTests() {
    ...
    console.log(
        "  ✔ AuthService.signUp dispatches POST /auth/signup and parses created user",
    );
    ...
    console.log(
        "  ✔ AuthService.signUp maps 409 Conflict error on duplicate email",
    );
    ...
    console.log(
        "  ✔ AuthService.signIn dispatches POST /auth/signin and returns JWT token",
    );
    ...
    console.log(
        "  ✔ AuthService.signIn handles 401 Unauthorized for invalid credentials",
    );
}
...
async function runBearerTokenTests() {
    ...
    console.log(
        "  ✔ authenticatedRequest correctly formats and injects Authorization: Bearer header",
    );
}
...
async function runChatServiceAuthTests() {
    ...
    globalThis.fetch = async (url: RequestInfo | URL, init?: RequestInit) => {
            ...
            const msgs: BackendMessage[] = [
                {
                    ...
                    conversation_id: 10,
                    sender_role: "user",
                    content: "What is Python experience?",
                },
                {
                    id: 2,
                    conversation_id: 10,
                    sender_role: "assistant",
                    content: "Found verified Python skills.",
                    artifacts: [
                        {
                            format: "svg",
                            data: '<svg><rect width="10" height="10" /></svg>',
                            encoding: "text",
                            commentary: "Evidence coverage chart",
                            evidence: [],
                        },
                    ],
                },
        ...
        if (urlStr.endsWith("/conversations/10/chat") && method === "POST") {
            const assistantMsg: BackendMessage = {
                ...
                sender_role: "assistant",
                content:
                    "Mariam Agent: Tariq Mansour has extensive Neo4j and FastAPI graph engineering records.",
                ...
                JSON.stringify({
                    ...
                    response: {
                        ...
                        artifacts: [
                            {
                                format: "svg",
                                data: '<svg><rect width="20" height="20" /></svg>',
                                encoding: "text",
                                commentary: "Evidence coverage chart",
                                evidence: [],
                            },
    ...
    assert.strictEqual(recordedCalls.length, 1);
    assert.strictEqual(
        recordedCalls[0].url.endsWith(`/users/${testUserId}/conversations`),
        true,
    );
    assert.strictEqual(
        recordedCalls[0].headers["Authorization"],
        `Bearer ${testToken}`,
    );
    assert.strictEqual(sessions.length, 2);
    assert.strictEqual(sessions[0].id, "10");
    console.log(
        "  ✔ ChatService.getSessions scopes sessions to authenticated user ID with Bearer header",
    );
    ...
    console.log(
        "  ✔ ChatService.createSession creates session scoped to authenticated user ID with Bearer header",
    );
    ...
    assert.strictEqual(recordedCalls.length, 1);
    assert.strictEqual(
        recordedCalls[0].url.endsWith("/conversations/10/messages"),
        true,
    );
    assert.strictEqual(
        recordedCalls[0].headers["Authorization"],
    ...
    assert.strictEqual(messages[1].artifacts?.[0].type, "svg");
    assert.strictEqual(
        messages[1].artifacts?.[0].content?.includes("<svg>"),
        true,
    );
    console.log(
        "  ✔ ChatService.getMessages sends Bearer header on conversation messages fetch",
    );
    ...
    assert.strictEqual(recordedCalls.length, 1);
    assert.strictEqual(
        recordedCalls[0].url.endsWith("/conversations/10/chat"),
        true,
    );
    assert.strictEqual(
        recordedCalls[0].headers["Authorization"],
    ...
    assert.strictEqual(chatResult.assistantResponse.artifacts?.[0].type, "svg");
    console.log(
        "  ✔ ChatService.sendMessage connects to live /chat endpoint with Bearer auth",
    );
}
...
Analysis complete.
  `.trim();
    ...
    const { cleanContent, extractedArtifacts } =
        extractArtifactsFromContent(sampleWithSvg);
    ...
    assert(
        !cleanContent.includes("<svg"),
        "Clean content should not contain raw inline SVG tags",
    );
    assert(cleanContent.includes("Here is the learner competency graph:"));
    assert(cleanContent.includes("Analysis complete."));
    console.log(
        "  ✔ extractArtifactsFromContent extracts inline SVG into VisualArtifact cleanly",
    );
...
\`\`\`
  `.trim();
    ...
    assert(
        codeResult.extractedArtifacts[0].content?.includes('<rect width="200"'),
    );
    assert(!codeResult.cleanContent.includes("```svg"));
    console.log(
        "  ✔ extractArtifactsFromContent extracts fenced ```svg``` code blocks",
    );
...
Candidate verified.
  `.trim();
    ...
    assert.strictEqual(
        imgResult.extractedArtifacts[0].url,
        "https://learner-os.ai/badges/verified-backend.png",
    );
    assert.strictEqual(
        imgResult.extractedArtifacts[0].title,
        "Candidate Verification Card",
    );
    assert(!imgResult.cleanContent.includes("![Candidate"));
    console.log(
        "  ✔ extractArtifactsFromContent extracts markdown image cards",
    );
    ...
    assert(
        !encodedDataUri.includes("<image"),
        "data-URI must percent-encode angle brackets preventing inline DOM injection",
    );
    assert(
        encodedDataUri.includes("%3Cimage"),
        "data-URI must percent-encode < characters",
    );
    console.log(
        "  ✔ XSS-safety: SVG with event handler payload is percent-encoded as data-URI, never injected as live DOM markup",
    );
}
...
async function runNetworkFallbackTests() {
    ...
    try {
        ...
        assert.strictEqual(
            err.isNetworkError,
            true,
            "Should have isNetworkError=true for fetch failure",
        );
        assert.strictEqual(
            err.status,
            0,
            "Network errors should have status 0",
        );
        assert(lastUrl.length > 0, "Should have attempted fetch");
    }
    console.log(
        "  ✔ Network connection failure correctly classifies as isNetworkError=true (fallback enabled)",
    );
    ...
    try {
        ...
        assert.strictEqual(
            err.isNetworkError,
            false,
            "HTTP 500 should NOT be isNetworkError — show real error, not offline mode",
        );
        assert.strictEqual(err.status, 500);
    }
    console.log(
        "  ✔ HTTP 500 error correctly classifies as isNetworkError=false (real error shown, offline mode NOT triggered)",
    );
}
...
async function runInvalidSessionTokenTests() {
    ...
    // Token without sub
    const payloadNoSub = Buffer.from(
    ...
    try {
        ...
        assert(
            ...
            "Must not report as wrong password",
        );
    }
    console.log(
        '  ✔ Token without sub displays "Sign-in failed: the server returned an invalid session. Please try again."',
    );

    // Token with sub=0
    const payloadSubZero = Buffer.from(JSON.stringify({ sub: 0 })).toString(
        "base64",
    );
    ...
    console.log(
        "  ✔ Token with sub=0 displays invalid session error rather than wrong password",
    );
    ...
    console.log(
        '  ✔ Real 401 correctly displays "Invalid email or password. Please check your credentials and try again."',
    );
}
