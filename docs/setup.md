...
`src/app/graph/constraints.py` (generated by `scripts/generate_constraints.py` -
do not hand-edit it) holds the constraints and indexes, plus
`initialize_schema(driver)` to apply them. It is safe to call more than
...
`MERGE`) and runs as one write transaction - nothing is left partially
committed if a batch fails partway through.

### Running the Graph Integration Tests

These tests exercise a real Neo4j instance rather than mocking it, and are
skipped automatically if one isn't reachable:
