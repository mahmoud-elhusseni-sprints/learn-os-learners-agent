...
def _extract_score_and_topic(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    topic_title = (
        record.get("topic_title") or task.get("description") or task.get("headline", "")
    )
...
def extract_lms_assessments(
    ...
    if answers_filepath_or_records is not None:
        raw_answers = []
        if isinstance(answers_filepath_or_records, str) and os.path.exists(
            answers_filepath_or_records
        ):
        ...
        for ans in raw_answers:
            ...
            learner_cards = ans.get("mastery_memory_cards") or cards_map.get(
                learner_id, []
            )
    ...
    if mock_assessments is not None:
        ...
        for mock_idx, mock in enumerate(mock_assessments):
            ...
            if not lx_id or not learner_id:
                logger.warning(f"Malformed mock assessment missing ID: {mock}")
                continue

            dedup_key = (learner_id, lx_id)
            if dedup_key in seen_unique_keys:
                logger.info(f"Idempotent skip: Duplicate mock key {dedup_key}")
                continue

            if target_lx_set and lx_id not in target_lx_set:
                continue

            matched_config = config_map.get((lx_id, learner_id)) or config_map.get(
                lx_id, {}
            )
            ...
            final_max_score = (
                mock.get("max_score") or score_topic_data["max_score"] or 100.0
            )
        ...
        for record in processed_records:
            ...
            if not lx_id or not learner_id:
                logger.warning(f"Skipping malformed config record missing ID: {record}")
                continue

            dedup_key = (learner_id, lx_id)
            if dedup_key in seen_unique_keys:
                logger.info(f"Idempotent skip: Duplicate config key {dedup_key}")
                continue
